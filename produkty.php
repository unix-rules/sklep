<?php
  ...
  if($numrows == 0)
  {
    echo "<h1>Brak produktu</h1>";
    echo "Kategoria nie zawiera produktów.";
  }
  else
  {
    ...
    while($prodrow = mysql_fetch_assoc($prodcatres))
    {
      echo "<tr>";
        if(empty($prodrow['obraz'])) {
          echo "<td><img src='./obraz/brak_obrazu.jpg' alt='" . $prodrow['nazwa'] . "'></td>";
        }
        else {
          echo "<td><img src='./obraz/" . $prodrow['obraz'] . "' alt='" . $prodrow['nazwa'] . "'></td>";
        }
        echo "<td>";
